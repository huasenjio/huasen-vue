...
<template>
  <div ref="vdr" class="vdr" :style="positionStyle" :class="`${active || isActive ? 'active' : 'inactive'} ${contentClass ? contentClass : ''}`" @mousedown="bodyDown($event)" @touchstart="bodyDown($event)" @touchend="up($event)">
    ...
    <div v-for="stick in sticks" :key="stick" class="vdr-stick" :class="['vdr-stick-' + stick, isResizable ? '' : 'not-resizable']" @mousedown.stop.prevent="stickDown(stick, $event)" @touchstart.stop.prevent="stickDown(stick, $event)" :style="stickStyle(stick)"></div>
    <div class="vdr-rotate" @mousedown.stop.prevent="rotateDown" @touchstart.stop.prevent="rotateDown"></div>
  </div>
</template>
<script>
const styleMapping = {
...
export default {
  ...
  mounted() {
    ...
    this.left = this.x;
    this.top = this.y;
    this.right = this.parentWidth - (this.w === 'auto' ? this.$refs.vdrContainer.scrollWidth : this.w) - this.left;
    this.bottom = this.parentHeight - (this.h === 'auto' ? this.$refs.vdrContainer.scrollHeight : this.h) - this.top;
  ...
  methods: {
    ...
    bodyMove(delta) {
      ...
      ({ newLeft: this.left, newRight: this.right, newTop: this.top, newBottom: this.bottom } = this.rectCorrectionByLimit({ newLeft, newRight, newTop, newBottom }));
    ...
    rotateMove(ev) {
      ...
      centerPoint.y = targetRect.y + targetRect.height / 2;
      const x = Math.abs(ev.x - centerPoint.x);
      const y = Math.abs(ev.y - centerPoint.y);
      const z = Math.sqrt(x * x + y * y);
      // 求出角度
      let rotate = Math.round((Math.asin(y / z) / Math.PI) * 180);
      // 第一象限
      if (ev.x >= centerPoint.x && ev.y <= centerPoint.y) {
        rotate = 90 - rotate;
      }
      // 第二象限
      else if (ev.x <= centerPoint.x && ev.y <= centerPoint.y) {
        rotate = 270 + rotate;
      }
      // 第三象限
      else if (ev.x <= centerPoint.x && ev.y >= centerPoint.y) {
      ...
      this.rotate = rotate === 360 ? 0 : parseInt(rotate);
    },
    rotateUp(ev) {
      this.rotateDrag = false;
    },
  ...
  watch: {
    ...
    x: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || this.rotateDrag || newVal === this.left) {
          return;
        }
    ...
    y: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || this.rotateDrag || newVal === this.top) {
          return;
        }
        const delta = oldVal - newVal;
        this.bodyDown({ pageX: this.left, pageY: this.top });
    ...
    r: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || this.rotateDrag || newVal === this.rotate) {
          return;
        }
        this.rotateDrag = true;
        this.rotate = newVal;
    ...
    w: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || this.rotateDrag || newVal === this.width) {
          return;
        }
        const stick = 'mr';
        const delta = oldVal - newVal;
    ...
    h: {
      handler(newVal, oldVal) {
        if (this.stickDrag || this.bodyDrag || this.rotateDrag || newVal === this.height) {
          return;
        }
        const stick = 'bm';
        const delta = oldVal - newVal;
...
.vdr.active:before {
  ...
  height: 100%;
  box-sizing: border-box;
  outline: 1px dashed var(--gray-100);
}
.inactive .vdr-stick {
  display: none;
}
